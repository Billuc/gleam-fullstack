...
function decode3(constructor, t1, t2, t3) {
  return (value3) => {
    ...
    if ($.isOk() && $1.isOk() && $2.isOk()) {
      ...
      let c = $2;
      return new Error(
        concat3(toList([all_errors(a), all_errors(b), all_errors(c)]))
      );
    }
  };
}
...
var ServerSentItems = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
// build/dev/javascript/app/components/new_item.mjs
function new_item() {
  let handle_click = (e) => {
  ...
  return div(
    toList([class$("bg-neutral-800 p-2 rounded-md w-fit mx-auto")]),
    toList([
      input(
        toList([
          class$(
...
// build/dev/javascript/app/components/shopping_item.mjs
function shopping_item(item) {
  let handle_amount_input = (e) => {
    let _pipe = value2(e);
      ...
      label(
        ...
        toList([
          ...
          input(
            toList([
              type_("number"),
              value(to_string2(item.amount)),
              class$(
                "w-1/3 pl-2 rounded-sm bg-white/50 hover:bg-white/75"
              ),
...
// build/dev/javascript/app/components/shopping_list.mjs
function shopping_list(model) {
  return keyed(
    (_capture) => {
      return div(
        toList([class$("flex flex-col gap-2 pb-2")]),
        _capture
      );
    },
    map3(
...
function update(model, msg) {
  ...
  } else if (msg instanceof QuantityChanged) {
    let id = msg.id;
...
function view(model) {
  return div(
    toList([
      class$("mx-auto p-4 bg-yellow-200 w-fit mt-16 rounded-md")
    ]),
    toList([
      h1(
        toList([class$("text-xl font-bold text-center mb-4")]),
        toList([text2("My shopping list")])
      ),
      shopping_list(model),
      new_item()
    ])
